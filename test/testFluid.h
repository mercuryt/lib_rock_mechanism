...
TEST_CASE("Fluid Groups merge")
{
	...
	CHECK(block3.volumeOfFluidTypeContains(s_water) == 100);
...
TEST_CASE("Denser fluids sink")
{
	...
	CHECK(fg1->m_fillQueue.m_set.size() == 0);
	fg1->readStep();
	fg2->readStep();
	...
	CHECK(fg1->m_fillQueue.m_queue[0].block == &block2);
	CHECK(fg1->m_fillQueue.m_queue[1].block == &block3);
	fg2->readStep();
	fg1->mergeStep();
	fg2->mergeStep();
	fg1->writeStep();
