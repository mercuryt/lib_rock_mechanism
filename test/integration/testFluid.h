...
TEST_CASE("Fluid Groups merge")
{
	...
	origin2.addFluid(100, s_water);
	CHECK(area.m_fluidGroups.size() == 2);
	FluidGroup<Block, Area>* fg1 = origin1.getFluidGroup(s_water);
	FluidGroup<Block, Area>* fg2 = origin2.getFluidGroup(s_water);
	CHECK(fg1 != fg2);
	// Step 1.
	fg1->readStep();
	fg2->readStep();
...
TEST_CASE("Fluid Groups merge four blocks")
{
	...
	CHECK(area.m_fluidGroups.size() == 2);
	FluidGroup<Block, Area>* fg1 = block1.getFluidGroup(s_water);
	FluidGroup<Block, Area>* fg2 = block4.getFluidGroup(s_water);
	CHECK(fg1 != fg2);
	// Step 1.
	fg1->readStep();
	fg2->readStep();
...
TEST_CASE("Denser fluids sink")
{
	...
	CHECK(area.m_fluidGroups.size() == 2);
	FluidGroup<Block, Area>* fgWater = block1.getFluidGroup(s_water);
	FluidGroup<Block, Area>* fgMercury = block2.getFluidGroup(s_mercury);
...
TEST_CASE("Merge 3 groups at two block distance")
{
	...
	FluidGroup<Block, Area>* fg1 = block1.getFluidGroup(s_water);
	FluidGroup<Block, Area>* fg2 = block4.getFluidGroup(s_water);
	FluidGroup<Block, Area>* fg3 = block7.getFluidGroup(s_water);
	CHECK(fg1 != nullptr);
	CHECK(fg2 != nullptr);
	CHECK(fg3 != nullptr);
...
TEST_CASE("Merge with two groups while spliting")
{
	...
	FluidGroup<Block, Area>* fg3 = origin5.getFluidGroup(s_water);
	CHECK(area.m_fluidGroups.size() == 3);
	CHECK(fg1 != fg2);
	CHECK(fg1 != fg3);
	CHECK(fg2 != fg3);
	...
	fg1->splitStep();
	fg2->splitStep();
	FluidGroup<Block, Area>* fg4 = block2.getFluidGroup(s_water);
	CHECK(!fg3->m_merged);
	fg3->splitStep();
	CHECK(fg1->m_drainQueue.m_set.size() == 2);
	CHECK(fg2->m_drainQueue.m_set.size() == 1);
...
TEST_CASE("Bubbles")
{
	...
	FluidGroup<Block, Area>* fg1 = origin1.getFluidGroup(s_CO2);
	FluidGroup<Block, Area>* fg2 = origin2.getFluidGroup(s_water);
	// Step 1.
	fg1->readStep();
	fg2->readStep();
...
TEST_CASE("Three liquids")
{
	...
	FluidGroup<Block, Area>* fg1 = origin1.getFluidGroup(s_CO2);
	FluidGroup<Block, Area>* fg2 = origin2.getFluidGroup(s_water);
	FluidGroup<Block, Area>* fg3 = origin3.getFluidGroup(s_mercury);
	// Step 1.
	fg1->readStep();
	fg2->readStep();
	fg3->readStep();
...
TEST_CASE("Set solid and split")
{
	...
	fg1->splitStep();
	CHECK(area.m_fluidGroups.size() == 2);
	FluidGroup<Block, Area>* fg2 = &area.m_fluidGroups.back();
	fg1->mergeStep();
	fg2->mergeStep();
	//Step 3.
	fg1->readStep();
...
TEST_CASE("Test diagonal seep")
{
	...
	FluidGroup<Block, Area>* fg1 = *area.m_unstableFluidGroups.begin();
	fg1->readStep();
	fg1->writeStep();
	fg1->afterWriteStep();
	fg1->splitStep();
	...
	s_step++;
	CHECK(block2.volumeOfFluidTypeContains(s_water) == 1);
	FluidGroup<Block, Area>* fg2 = block2.getFluidGroup(s_water);
	CHECK(fg1 != fg2);
	CHECK(fg1->m_excessVolume == -1);
	CHECK(!fg1->m_stable);
	for(int i = 0; i < 5; ++i)
...
void trenchTest2Fluids(uint32_t scaleL, uint32_t scaleW, uint32_t steps)
{
	...
	while(s_step < steps)
	{
		for(FluidGroup<Block, Area>* fluidGroup : area.m_unstableFluidGroups)
			fluidGroup->readStep();
		area.writeStep();
		s_step++;
	}
...
void trenchTest2FluidsMerge(uint32_t scaleL, uint32_t scaleW, uint32_t steps)
{
	...
	FluidGroup<Block, Area>* fgCO2 = water2.getFluidGroup(s_CO2);
	CHECK(fgWater->totalVolume() == totalVolume);
	CHECK(fgCO2->totalVolume() == totalVolume);
	CHECK(fgWater->m_drainQueue.m_set.size() == expectedBlocks);
	CHECK(fgCO2->m_drainQueue.m_set.size() == expectedBlocks);
...
void trenchTest3FluidsMerge(uint32_t scaleL, uint32_t scaleW, uint32_t steps)
{
	...
	CHECK(area.m_unstableFluidGroups.empty());
	fgWater = getFluidGroup(area, s_water);
	FluidGroup<Block, Area>* fgCO2 = getFluidGroup(area, s_CO2);
	FluidGroup<Block, Area>* fgMercury = getFluidGroup(area, s_mercury);
	CHECK(fgWater != nullptr);
	CHECK(fgCO2 != nullptr);
	CHECK(fgMercury != nullptr);
