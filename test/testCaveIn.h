...
TEST_CASE("Cave in does happen when block is not supported.")
{
	...
	area.stepCaveInWrite();
	CHECK(area.m_blocks[50][50][2].m_solid == nullptr);
	CHECK(area.m_blocks[50][50][1].m_solid == s_stone);
...
TEST_CASE("Blocks on the edge of the area are anchored.")
{
	...
	CHECK(block2->m_solid == s_stone);
	CHECK(block->m_adjacents[0]->m_solid == nullptr);
	CHECK(block2->m_adjacents[0]->m_solid == nullptr);
	CHECK(area.m_caveInCheck.size() == 0);
}
TEST_CASE("Verify recorded fall distance is the shortest.")
{
	...
	block->m_solid = block2->m_solid = block3->m_solid = s_stone;
	area.m_caveInCheck.insert(block);
	area.m_caveInCheck.insert(block2);
	...
	CHECK(block2->m_solid == nullptr);
	CHECK(block2->m_adjacents[0]->m_solid == s_stone);
	CHECK(block3->m_solid == nullptr);
	CHECK(block3->m_adjacents[0]->m_solid == s_stone);
	CHECK(area.m_caveInCheck.size() == 1);
}
TEST_CASE("Verify one group falling onto another unanchored group will keep falling in the next step.")
{
	...
	block->m_solid = block2->m_solid = s_stone;
	area.m_caveInCheck.insert(block);
	area.m_caveInCheck.insert(block2);
	area.stepCaveInRead();
	CHECK(area.m_caveInData.size() == 2);
