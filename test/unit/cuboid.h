...
TEST_CASE("merge")
{
	...
	BaseCuboid<Block, Actor, Area> sum = c1.sum(c2);
	CHECK(sum.size() == 8);
	CHECK(c1 != c2);
	c2.merge(c1);
	CHECK(c2.size() == 8);
...
{
	Area area(2,2,2);
	BaseCuboid<Block, Actor, Area> c1(&area.m_blocks[1][1][1], &area.m_blocks[0][0][0]);
	BaseCuboid<Block, Actor, Area> face = c1.getFace(4);
	CHECK(face.size() == 4);
