...
TEST_CASE("route_5_5_3")
{
	...
	SUBCASE("walking path blocked by water if not also swimming")
	{
		...
		actors.move_setDestination(actor, destination);
		REQUIRE(!blocks.shape_moveTypeCanEnter(midpoint, actors.move_getType(actor)));
		simulation.doStep();
		REQUIRE(actors.move_getPath(actor).empty());
...
TEST_CASE("route_5_5_5")
{
	...
	SUBCASE("walking path blocked by one height cliff if not climbing")
	{
		...
		actors.move_setDestination(actor, blocks.getIndex(4, 4, 2));
		simulation.doStep();
		REQUIRE(actors.move_getPath(actor).empty());
	...
	SUBCASE("walking path blocked by two height cliff if not climbing 2")
	{
		...
		actors.move_setDestination(actor, blocks.getIndex({4, 4, 3}));
		simulation.doStep();
		REQUIRE(actors.move_getPath(actor).empty());
	...
	SUBCASE("stairs")
	{
		...
		actors.move_setDestination(actor, blocks.getIndex({2, 2, 4}));
		simulation.doStep();
		REQUIRE(!actors.move_getPath(actor).empty());
	}
	SUBCASE("ramp")
	...
	SUBCASE("door")
	{
		...
		actors.move_setDestination(actor, blocks.getIndex({4, 3, 1}));
		simulation.doStep();
		REQUIRE(!actors.move_getPath(actor).empty());
	...
	SUBCASE("can walk on floor")
	{
		...
		actors.move_setDestination(actor, blocks.getIndex({1, 3, 2}));
		simulation.doStep();
		REQUIRE(actors.move_getPath(actor).empty());
	...
	SUBCASE("floor blocks vertical travel")
	{
		...
		actors.move_setDestination(actor, blocks.getIndex({3, 3, 3}));
		simulation.doStep();
		REQUIRE(!actors.move_getPath(actor).empty());
	...
	SUBCASE("multi-block actors can use ramps")
	{
		...
		actors.move_setDestination(actor, blocks.getIndex({3, 3, 2}));
		simulation.doStep();
		REQUIRE(actors.move_getPath(actor).empty());
	...
	SUBCASE("detour")
	{
		...
		REQUIRE(blocks.shape_shapeAndMoveTypeCanEnterEverFrom(blocks.getIndex({3, 3, 1}), actors.getShape(actor), actors.getMoveType(actor), actors.getLocation(actor)));
		REQUIRE(!blocks.shape_canEnterCurrentlyFrom(blocks.getIndex({3, 3, 1}), actors.getShape(actor), actors.getLocation(actor), actors.getBlocks(actor)));
		REQUIRE(actors.move_hasEvent(actor));
		// Move attempt 1.
