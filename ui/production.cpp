...
void NewProductionView::draw()
{
	...
	m_panel->add(confirm);
	confirm->onClick([&]{
		m_window.getArea()->m_hasCraftingLocationsAndJobs.getForFaction(m_window.getFaction()).
...
void ProductionView::draw()
{
	...
	size_t row = 0;
	auto& hasCrafting = m_window.getArea()->m_hasCraftingLocationsAndJobs.getForFaction(m_window.getFaction());
	for(auto [craftJobType, materialType, quantity, craftJob] : collate())
	{
		...
		if(quantity != 1)
			list->addWidget(tgui::Label::create(std::to_string(quantity.get())), row, 2);
...
std::vector<std::tuple<CraftJobTypeId, MaterialTypeId, Quantity, CraftJob*>> ProductionView::collate()
{
	std::vector<std::tuple<CraftJobTypeId, MaterialTypeId, Quantity, CraftJob*>> output;
	for(CraftJob& craftJob : m_window.getArea()->m_hasCraftingLocationsAndJobs.getForFaction(m_window.getFaction()).getAllJobs())
	{
