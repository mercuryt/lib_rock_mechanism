...
void HaulSubproject::commandWorker(Actor& actor)
{
	...
	switch(m_strategy)
	{
		case HaulStrategy::Individual:
			...
			if(actor.m_canPickup.m_carrying == &m_toHaul)
			{
				if(actor.m_location == &m_destination)
				{
					actor.m_canPickup.drop();
					onComplete();
				}
				else
...
bool HaulSubproject::tryToSetHaulStrategy(Actor& actor, std::unordered_set<Actor*>& waiting)
{
...
void CanPickup::pickUp(Item& item, uint8_t quantity)
{
...
void CanPickup::pickUp(Actor& actor)
{
...
{
	return (m_actor.maxCarryMass - m_actor.m_hasItems.getMass()) / (itemType.mass * materialType.density);
}
uint32_t CanPickup::getMass()
{
	return m_carrying.m_mass;
}
