MistDisperseEvent::MistDisperseEvent(uint32_t s, const FluidType* ft, DerivedBlock& b) :
		...
		if(m_block.m_mistInverseDistanceFromSource > 0)
			for(DerivedBlock* adjacent : m_block.m_adjacentsVector)
				  ...
				  )
				{
					...
					auto event = std::make_unique<MistDisperseEvent>( s_step + m_fluidType->mistDuration, m_fluidType, static_cast<DerivedBlock&>(*adjacent));
					m_block.m_area->m_eventSchedule.schedule(std::move(event));
				}
		// Schedule next check.
		auto event = std::make_unique<MistDisperseEvent>( s_step + m_fluidType->mistDuration, m_fluidType, static_cast<DerivedBlock&>(m_block));
		m_block.m_area->m_eventSchedule.schedule(std::move(event));
		return;
	}
