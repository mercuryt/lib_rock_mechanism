#pragma once
Cuboid::Cuboid(DerivedBlock* h, DerivedBlock* l) : m_highest(h), m_lowest(l)
{
...
bool Cuboid::contains(const DerivedBlock& block) const
{
	return (
...
Cuboid Cuboid::getFace(uint32_t facing) const
{
	...
	if(facing == 4)
		return Cuboid(m_highest, &m_highest->m_area->m_blocks[m_highest->m_x][m_lowest->m_y][m_lowest->m_z]);
	// test area has x lower then this.
	else if(facing == 2)
		return Cuboid(&m_highest->m_area->m_blocks[m_lowest->m_x][m_highest->m_y][m_highest->m_z], m_lowest);
	// test area has y higher then this.
	else if(facing == 3)
		return Cuboid(m_highest, &m_highest->m_area->m_blocks[m_lowest->m_x][m_highest->m_y][m_lowest->m_z]);
	// test area has y lower then this.
	else if(facing == 1)
		return Cuboid(&m_highest->m_area->m_blocks[m_highest->m_x][m_lowest->m_y][m_highest->m_z], m_lowest);
			// test area has z higher then this.
	else if(facing == 5)
		return Cuboid(m_highest, &m_highest->m_area->m_blocks[m_lowest->m_x][m_lowest->m_y][m_highest->m_z]);
	// test area has z lower then this.
	else if(facing == 0)
		return Cuboid(&m_highest->m_area->m_blocks[m_highest->m_x][m_highest->m_y][m_lowest->m_z], m_lowest);
...
Cuboid::iterator::iterator(Cuboid& c, const DerivedBlock& block) : cuboid(&c), x(block.m_x), y(block.m_y), z(block.m_z) {}
...
bool Cuboid::iterator::operator!=(const Cuboid::iterator other) const { return other.x != x || other.y != y || other.z != z; }
DerivedBlock& Cuboid::iterator::operator*() const { return cuboid->m_highest->m_area->m_blocks[x][y][z]; }
DerivedBlock* Cuboid::iterator::operator->() const { return &cuboid->m_highest->m_area->m_blocks[x][y][z]; }
...
Cuboid::const_iterator::const_iterator(const Cuboid& c, const DerivedBlock& block) : cuboid(&c), x(block.m_x), y(block.m_y), z(block.m_z) {}
...
bool Cuboid::const_iterator::operator!=(const Cuboid::const_iterator other) const { return other.x != x || other.y != y || other.z != z; }
const DerivedBlock& Cuboid::const_iterator::operator*() const { return cuboid->m_highest->m_area->m_blocks[x][y][z]; }
const DerivedBlock* Cuboid::const_iterator::operator->() const { return &cuboid->m_highest->m_area->m_blocks[x][y][z]; }
