...
FindPathResult SleepPathRequest::readStep(Area& area, const TerrainFacade& terrainFacade, PathMemoBreadthFirst& memo)
{
	...
	int desireToSleepAtCurrentLocation = m_sleepObjective.desireToSleepAt(area, actors.getLocation(actorIndex), actorIndex);
	...
	if(m_maxDesireCandidate.empty())
	{
		...
		if(faction.exists())
		{
			...
			auto condition = [&](const Point3D& point, const Facing4&) -> std::pair<bool, Point3D>
			{
				...
				{
					int desire = m_sleepObjective.desireToSleepAt(area, point, actorIndex);
					if(desire == 3)
					{
...
int SleepObjective::desireToSleepAt(Area& area, const Point3D& point, const ActorIndex& actor) const
{
	Space& space = area.getSpace();
	Actors& actors = area.getActors();
