...
struct Point3D
{
	...
	[[nodiscard]] constexpr Primitive get() const { return {data[0], data[1], data[2]}; }
	...
	[[nodiscard]] Distance distanceTo(const Cuboid cuboid) const;
...
inline void to_json(Json& data, const Point3D::Primitive& primitive) { data = {primitive.x, primitive.y, primitive.z}; }
inline void from_json(const Json& data, Point3D::Primitive& primitive)
{
	data[0].get_to(primitive.x);
	data[1].get_to(primitive.y);
	data[2].get_to(primitive.z);
}
...
GDB_CALLABLE Offset3D createOffset(const OffsetWidth& x, const OffsetWidth& y, const OffsetWidth& z);
GDB_CALLABLE Point3D createPoint(const DistanceWidth& x, const DistanceWidth& y, const DistanceWidth& z);