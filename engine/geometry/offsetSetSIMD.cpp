...
void OffsetSetSIMD::insert(const Blocks& blocks, const Offset3D& offset)
{
...
BlockIndexSetSIMD OffsetSetSIMD::getIndicesInBounds([[maybe_unused]] const Blocks& blocks, const BlockIndex& location) const
{
	Eigen::Array<int, 1, Eigen::Dynamic> output = (m_indexData + (int)location.get());
	assert((output >= 0 && output < blocks.size()).all());
	return output;
}
BlockIndexSetSIMD OffsetSetSIMD::getIndicesMaybeOutOfBounds(const Blocks& blocks, const BlockIndex& location) const
{
