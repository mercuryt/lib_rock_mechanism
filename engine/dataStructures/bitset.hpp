...
template<typename IntType, unsigned int capacity>
bool BitSet<IntType, capacity>::test(const uint8_t& index) const { assert(index < capacity); return (data >> index) & 1; }
template<typename IntType, unsigned int capacity>
...
template<typename IntType, unsigned int capacity>
void BitSet<IntType, capacity>::set(const uint8_t& index) { data |= (1 << index); }
template<typename IntType, unsigned int capacity>
void BitSet<IntType, capacity>::set(const uint8_t& index, bool value) { if(value) set(index); else unset(index); }
template<typename IntType, unsigned int capacity>
void BitSet<IntType, capacity>::unset(const uint8_t& index) { data &= ~(1 << index); }
template<typename IntType, unsigned int capacity>
