...
template<typename Contained, class Index>
uint StrongVector<Contained, Index>::size() const { return data.size(); }
template<typename Contained, class Index>
uint StrongVector<Contained, Index>::capacity() const { return data.capacity(); }
template<typename Contained, class Index>
...
template<typename Contained, class Index>
void StrongVector<Contained, Index>::sortRangeWithOrder(const Index& begin, const Index& end, std::vector<std::pair<uint32_t, Index>> sortOrder)
{
...
bool StrongBitSet<Index>::operator[](const Index& index) const { assert((uint)index.get() < data.size()); return data[index.get()]; }
template<class Index>
...
template<class Index>
void StrongBitSet<Index>::sortRangeWithOrder(const Index& begin, const Index& end, std::vector<std::pair<uint32_t, Index>> sortOrder)
{
