...
void from_json(const Json& data, ObjectiveTypeId& id) { id = ObjectiveType::getIdByName(data.get<std::wstring>()); }
#include "plantSpecies.h"
void to_json(Json& data, const PlantSpeciesId& id) { data = PlantSpecies::getName(id); }
void from_json(const Json& data, PlantSpeciesId& id) { id = PlantSpecies::byName(data.get<std::wstring>()); }
#include "animalSpecies.h"
void to_json(Json& data, const AnimalSpeciesId& id) { data = AnimalSpecies::getName(id); }
void from_json(const Json& data, AnimalSpeciesId& id) { id = AnimalSpecies::byName(data.get<std::wstring>()); }
#include "materialType.h"
void to_json(Json& data, const MaterialTypeId& id) { data = MaterialType::getName(id); }
void from_json(const Json& data, MaterialTypeId& id) { id = MaterialType::byName(data.get<std::wstring>()); }
void to_json(Json& data, const MaterialCategoryTypeId& id) { data = MaterialTypeCategory::getName(id); }
void from_json(const Json& data, MaterialCategoryTypeId& id) { id = MaterialTypeCategory::byName(data.get<std::wstring>()); }
#include "itemType.h"
...
void from_json(const Json& data, ShapeId& id) { id = Shape::byName(data.get<std::wstring>()); }
#include "skill.h"
void to_json(Json& data, const SkillTypeId& id) { data = SkillType::getName(id); }
void from_json(const Json& data, SkillTypeId& id) { id = SkillType::byName(data.get<std::wstring>()); }
#include "moveType.h"
void to_json(Json& data, const MoveTypeId& id) { data = MoveType::getName(id); }
void from_json(const Json& data, MoveTypeId& id) { id = MoveType::byName(data.get<std::wstring>()); }
#include "bodyType.h"
void to_json(Json& data, const BodyTypeId& id) { data = BodyType::getName(id); }
void from_json(const Json& data, BodyTypeId& id) { id = BodyType::byName(data.get<std::wstring>()); }
void to_json(Json& data, const BodyPartTypeId& id) { data = BodyPartType::getName(id); }
void from_json(const Json& data, BodyPartTypeId& id) { id = BodyPartType::byName(data.get<std::wstring>()); }
#include "craft.h"
void to_json(Json& data, const CraftStepTypeCategoryId& id) { data = CraftStepTypeCategory::getName(id); }
void from_json(const Json& data, CraftStepTypeCategoryId& id) { id = CraftStepTypeCategory::byName(data.get<std::wstring>()); }
void to_json(Json& data, const CraftJobTypeId& id) { data = CraftJobType::getName(id); }
void from_json(const Json& data, CraftJobTypeId& id) { id = CraftJobType::byName(data.get<std::wstring>()); }
#include "attackType.h"
void to_json(Json& data, const AttackTypeId& id) { data = AttackType::getName(id); }
void from_json(const Json& data, AttackTypeId& id) { id = AttackType::byName(data.get<std::wstring>()); }
