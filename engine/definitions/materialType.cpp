...
uint32_t MaterialType::getHardness(const MaterialTypeId& id) { return g_materialTypeData.m_hardness[id]; }
bool MaterialType::getTransparent(const MaterialTypeId& id) { return g_materialTypeData.m_transparent[id]; }
uint MaterialType::getValuePerUnitFullDisplacement(const MaterialTypeId& id) { return g_materialTypeData.m_valuePerUnitFullDisplacement[id]; }
...
Mass MaterialType::getMassForSolidVolumeAsANumberOfPoints(const MaterialTypeId& id, uint32_t numberOfPoints) { return {(uint32_t)(getDensity(id).get() * Config::maxPointVolume.get()) * numberOfPoints};}
// Fire.
