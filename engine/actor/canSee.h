...
public:
	...
	void doVision(std::vector<Actor*>& actors);
	void setVisionRange(DistanceInBlocks range);
	void createFacadeIfCanSee();
	[[nodiscard]] std::vector<Actor*>& getCurrentlyVisibleActors() { return m_currently; }
	[[nodiscard]] DistanceInBlocks getRange() const { return m_range; }
	[[nodiscard]] bool isCurrentlyVisible(Actor& actor) const;
