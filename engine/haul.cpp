	...
	m_projectRequirementCounts(*paramaters.projectRequirementCounts)
{
	...
	if(paramaters.beastOfBurden.exists())
	{
		...
		std::unique_ptr<DishonorCallback> dishonorCallback = std::make_unique<HaulSubprojectDishonorCallback>(*this);
		items.reservable_reserve(m_beastOfBurden.getIndex(), m_project.m_canReserve, 1u, std::move(dishonorCallback));
	}
		...
		case HaulStrategy::Team:
			...
			if(m_itemIsMoving)
			{
				...
				if(actors.isAdjacentToLocation(actor, m_project.m_location))
				{
					...
					toHaulIndex.setLocationAndFacing(area, actorLocation, 0);
					complete(toHaulIndex);
				}
				else
					actors.move_setDestinationAdjacentToLocation(actor, m_project.m_location, detour);
			...
			else
			{
				...
				if(m_liftPoints.contains(ref))
				{
					...
					if(actorLocation == m_liftPoints.at(ref))
					{
						...
						actors.move_setDestination(m_liftPoints.at(ref), detour);
				}
				else
				{
			...
			if(actors.isLeadingItem(actor, haulToolIndex))
			{
				...
				if(hasCargo)
				{
					...
					if(actors.isAdjacentToLocation(actor, m_project.m_location))
					{
						...
						{
							ItemIndex item = items.cargo_unloadGenericItemToLocation(haulToolIndex, *m_genericItemType, *m_genericMaterialType, m_quantity, actorLocation);
			...
			if(actors.equipment_containsItem(m_beastOfBurden.getIndex(), m_haulTool.getIndex()))
			{
				...
				if(hasCargo)
				{
					...
					if(actors.isAdjacentToLocation(actor, m_project.m_location))
					{
						...
						{
							ItemIndex item = items.cargo_unloadGenericItemToLocation(m_haulTool.getIndex(), *m_genericItemType, *m_genericMaterialType, m_quantity, actorLocation);
							delivered = ActorOrItemIndex::createForItem(item);
						}
						// TODO: set rotation?
...
Speed HaulSubproject::getSpeedWithHaulToolAndCargo(const Area& area, const ActorIndex leader, const ItemIndex haulTool, const ActorOrItemIndex toHaul, Quantity quantity)
{
	...
	return Portables::getMoveSpeedForGroupWithAddedMass(area, actorsAndItems, toHaul.getSingleUnitMass(area) * quantity, 0);
}
// Class method.
...
ActorIndices HaulSubproject::actorsNeededToHaulAtMinimumSpeedWithTool(const Project& project, ActorIndex leader, const ActorOrItemIndex toHaul, const ItemIndex haulTool)
{
	...
	for(auto& [actor, projectWorker] : project.m_workers)
	{
		...
		Speed speed = Portables::getMoveSpeedForGroupWithAddedMass(project.m_area, actorsAndItems, mass, 0);
		if(speed >= project.getMinimumHaulSpeed())
			return output;
	}
	// Cannot form a team to move at acceptable speed.
...
Speed HaulSubproject::getSpeedWithPannierBearerAndPanniers(const Area& area, const ActorIndex leader, const ActorIndex pannierBearer, const ItemIndex panniers, const ActorOrItemIndex toHaul, Quantity quantity)
{
	...
	// actorsAndItems, rollingMass, deadMass
	return Portables::getMoveSpeedForGroupWithAddedMass(area, actorsAndItems, 0, (toHaul.getSingleUnitMass(area) * quantity) + area.getItems().getMass(panniers));
}
// Class method.
...
void AreaHasHaulTools::unregisterHaulTool(const Area& area, ItemIndex item)
{
	const ItemReference ref = area.getItems().getReferenceConst(item); 
	assert(m_haulTools.contains(ref));
	m_haulTools.remove(ref);
}
void AreaHasHaulTools::unregisterYokeableActor(const Area& area, ActorIndex actor)
{
	ActorReference ref = area.getActors().getReference(actor); 
	assert(m_yolkableActors.contains(ref));
