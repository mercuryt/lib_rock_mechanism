...
public:
	...
	[[nodiscard]] uint32_t getOffsetForDesignation(BlockDesignation designation) const { return (uint)designation * m_areaSize.get(); }
	[[nodiscard]] bool checkWithOffset(uint32_t offset, BlockIndex index) const { return m_designations[offset + index.get()]; }
