...
public:
	VisionCuboidSetSIMD(int capacity);
...
public:
	...
	[[nodiscard]] int getIndexForVisionCuboidId(const VisionCuboidId& index) const;
	[[nodiscard]] Cuboid getCuboidByVisionCuboidId(const VisionCuboidId& index) const;
	[[nodiscard]] Cuboid getCuboidByIndex(const int& index) const { return m_cuboids[index]; }
	...
	[[nodiscard]] CuboidMap<VisionCuboidId> getAdjacentsForIndex(const int& index) const { return m_adjacent[index]; }
	...
	[[nodiscard]] SmallSet<int> getMergeCandidates(const Cuboid& cuboid) const;
