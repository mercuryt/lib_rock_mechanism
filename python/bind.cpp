...
NB_MODULE(bound, m)
{
	...
	nb::class_<Area>(m, "Area")
		.def("getPoint", &Area::getPoint, nb::rv_policy::reference)
	...
	nb::class_<Point3D>(m, "Point3D")
		.def("isSolid", &Point3D::isSolid)
		.def("getSolidMaterial", &Point3D::getSolidMaterial, nb::rv_policy::reference)
		.def_ro("x", &Point3D::m_x)
		.def_ro("y", &Point3D::m_y)
		.def_ro("z", &Point3D::m_z)
		.def_ro("exposedToSky", &Point3D::m_exposedToSky)
		.def_ro("underground", &Point3D::m_underground)
		.def_ro("outdoors", &Point3D::m_outdoors)
		.def_ro("totalFluidVolume", &Point3D::m_totalFluidVolume)
		.def("getActors", [](Point3D& point){ return point.m_hasActors.getAll(); }, nb::rv_policy::reference)
		.def("getItems", [](Point3D& point){ return point.m_hasItems.getAll(); }, nb::rv_policy::reference)
		.def("getPlant", [](Point3D& point) -> Plant& { return point.m_hasPlant.get(); }, nb::rv_policy::reference)
		.def_ro("getFluids", &Point3D::m_fluids, nb::rv_policy::reference)
		.def("addFluid", &Point3D::addFluid);
	// CauseOfDeath
	...
	nb::class_<Actor>(m, "Actor")
		...
		.def("setDestination",[](Actor& actor, Point3D& destination){ actor.m_canMove.setDestination(destination); })
		.def("setDestinationAdjacentTo",[](Actor& actor, Point3D& destination){ actor.m_canMove.setDestinationAdjacentTo(destination); })
	...
	nb::class_<Item>(m, "Item")
		...
		.def("materialType", [](Item& item){ return item.m_solid; })
